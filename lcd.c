...
# define maskbits1 0xf0 // This is the maskbits for doing several things

/*
  lcd_init - Do various things to initialize the LCD display
*/
void lcd_init(void) {
  //Set Group B - Bits 0 and 1 as outputs
    DDRB |= (1 << 0);
    DDRB |= (1 << 1);
  //Set Group B - Bit 2 as an input
    DDRB &= ~(1 << 2);
  //Set Group D - Bits 4 through 7 as outputs
    DDRD |= (1 << 4);
    DDRD |= (1 << 5);
    DDRD |= (1 << 6);
    DDRD |= (1 << 7);
...
void lcd_writenibble(unsigned char lcdbits) {
    ...
    PORTD &= ~(maskbits1);
    PORTD |= (lcdbits & maskbits1);

    /* Make E signal (PB1) go to 1 and back to 0 */
    PORTB |= (1 << 1); // Set E to 1
    PORTB |= (1 << 1); //Make Longer
    PORTB &= ~(1 << 1); // Clear E to 0
